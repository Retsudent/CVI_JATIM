<?php
// If controller/router passed a normalized $event from DB, use it directly
if (isset($event) && is_array($event) && !empty($event)) {
    // when using DB-backed detail, $event already contains all required fields
    $event_id = $event_id ?? 'db';
} else {
// Event data based on ID (static fallback examples)
...
// Ensure $event is an array and has expected keys to avoid Undefined array key errors
if (!is_array($event)) {
    $event = (array) $event;
}
...
$defaults = [
    'badge_class'     => 'bg-secondary',
    // preserve provided badge_text if present; fallback to capitalized status or 'Info'
    'badge_text'      => isset($event['badge_text']) ? $event['badge_text'] : ucfirst((string)($event['status'] ?? 'info')),
    'date'            => $event['date'] ?? '',
    'title'           => $event['title'] ?? '',
...
// Merge defaults but coerce certain keys to safe types to avoid null overwrites
$event = array_merge($defaults, $event);

// If DB provided these keys as null, ensure they're the correct types
if (!is_array($event['activities'])) {
    $event['activities'] = [];
}
...
$bannerSrc    = isset($event['image']) ? $event['image'] : '';
$localGallery = [];
try {
    $candidateDirs = [
    ...
    foreach ($candidateDirs as $dir) {
        ...
        if (!empty($matchesId)) {
            // Preferred: a local file that starts with the event id (event-specific)
            $bannerSrc = base_url(str_replace(FCPATH, '', $matchesId[0]));
        } elseif (!empty($matchesAny) && empty($event['image'])) {
            // Fallback: only use a generic local image when the event has no configured image
        ...
        <div class="card mb-4" style="background: linear-gradient(135deg, #f8fff8 0%, #e6ffe6 100%); border: none;">
            <div class="card-body p-4">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <?php
                    // defensive defaults in case $event does not include these keys (DB-backed events may omit them)
                    $badge_class = isset($event['badge_class']) ? $event['badge_class'] : 'bg-secondary';
                    $badge_text = isset($event['badge_text']) ? $event['badge_text'] : ucfirst((string)($event['status'] ?? 'info'));
                    ?>
                    <span class="badge <?= $badge_class ?> fs-6 px-3 py-2"><?= $badge_text ?></span>
                ...
                <div class="text-center">
                    <?php $wa = $event['whatsapp_contact'] ?? '';
                    $wa = preg_replace('/[^0-9]/', '', (string)$wa);
                    if ($wa === '') { $wa = '6283134846000'; }
                    ?>
                    <a href="https://wa.me/<?= $wa ?>?text=Halo, saya tertarik dengan event <?= urlencode($event['title']) ?>" 
...
<!-- Related Events (only for static example data) -->
            ...
            <div class="col-lg-4 col-md-6">
                <div class="card h-100">
                    <div class="card-body">
                        ...
                        <p class="card-text"><?= substr($related_event['description'], 0, 100) ?>...</p>
