...
<div class="admin-form-container">
    <div class="admin-form-header">
        <h2>✏️ Edit Campground</h2>
        <p class="subtitle">Update informasi campground yang sudah ada</p>
    </div>
    
    <form method="post" action="/admin/campground/edit/<?= htmlspecialchars($camp['id']) ?>" enctype="multipart/form-data">
        <?= csrf_field() ?>
        <div class="form-grid">
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="<?= htmlspecialchars($camp['name']) ?>" required placeholder="Contoh: Wirotaman Premium Campground" />
                    <div class="form-help">Buat nama yang menarik dan mudah diingat</div>
                </div>
                
                <div class="form-group">
                    <label>Deskripsi <span class="required">*</span></label>
                    <textarea name="description" required placeholder="Jelaskan keunggulan campground, pemandangan, dan pengalaman yang akan didapat pengunjung..."><?= htmlspecialchars($camp['description']) ?></textarea>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="location" value="<?= htmlspecialchars($camp['location']) ?>" required placeholder="Contoh: Desa Wirotaman, Malang" />
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="price_per_person" value="<?= htmlspecialchars($camp['price_per_person']) ?>" required placeholder="25000" />
                ...
                <div class="form-group">
                    ...
                    <select name="status">
                        <option value="active" <?= $camp['status']==='active'?'selected':'' ?>>✅ Active - Campground aktif</option>
                        <option value="inactive" <?= $camp['status']==='inactive'?'selected':'' ?>>❌ Inactive - Campground tidak aktif</option>
                        <option value="maintenance" <?= $camp['status']==='maintenance'?'selected':'' ?>>🔧 Maintenance - Sedang perbaikan</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="number" name="capacity_tent" min="0" value="<?= htmlspecialchars($camp['capacity_tent']) ?>" placeholder="50" />
                    <div class="form-help">Jumlah maksimal tenda yang bisa dipasang</div>
                </div>
                
                <div class="form-group">
                    <label>Kapasitas Orang</label>
                    <input type="number" name="capacity_people" min="0" value="<?= htmlspecialchars($camp['capacity_people']) ?>" placeholder="150" />
                    <div class="form-help">Jumlah maksimal pengunjung</div>
                </div>
                
                <div class="form-group">
                    <label>Kapasitas Parkir</label>
                    <input type="number" name="capacity_parking" min="0" value="<?= htmlspecialchars($camp['capacity_parking']) ?>" placeholder="30" />
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <textarea name="address" placeholder="Contoh: Jl. Wirotaman No. 123, Desa Wirotaman, Kecamatan Malang, Jawa Timur 65100"><?= htmlspecialchars($camp['address']) ?></textarea>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="coordinates_lat" value="<?= htmlspecialchars($camp['coordinates_lat']) ?>" placeholder="-7.9775" />
                    <div class="form-help">Koordinat lintang (dapat dari Google Maps)</div>
                </div>
                
                <div class="form-group">
                    <label>Longitude</label>
                    <input type="text" name="coordinates_lng" value="<?= htmlspecialchars($camp['coordinates_lng']) ?>" placeholder="112.6340" />
...
<script>
function previewCampEditImage(input) {
    var file = input.files && input.files[0];
    if (!file) return;
    var reader = new FileReader();
    reader.onload = function(e) {
        var p = document.getElementById('camp-edit-image-preview');
        var img = document.getElementById('camp-edit-preview-img');
